...
export default function TaskCard({ task }: TaskCardProps) {
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div
        className="flex items-center gap-[15px] bg-background rounded-lg px-[18px] py-[10px] cursor-pointer border border-transparent hover:shadow-[0_-2px_8px_rgba(0,0,0,0.06),0_4px_12px_rgba(0,0,0,0.08)] hover:border-primary/20 transition-colors transition-shadow duration-200 ease-out"
        onClick={handleClick}
        role="button"
        tabIndex={0}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {hasMetaLine && (
            <div className="flex items-center gap-[10px] flex-wrap text-[11px] <div className="flex items-center gap-[10px] flex-wrap text-[11px] text-muted-foreground mt-[4px]">">
              {task.delegatedFrom && (
              ...
              {task.delegatedFrom && task.groupName && (
                <span className="<span className="text-muted-foreground/60">|</span>">|</span>
              )}
              {task.groupName && (
                <span className="flex items-center gap-1">
